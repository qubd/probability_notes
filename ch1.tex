...
\end{examp}
\par
Using the definition of $P(A \given B)$, we can state what it means for two events to be independent in a different way. The result is equivalent to, and sometimes used in place of, Definition \ref{independentevents} above.
$$P(A) = P(A \given B) \biimp P(A) = \frac{P(A \cap B)}{P(B)} \biimp P(A)P(B) = P(A \cap B)$$

\index{Events!independent}
\begin{thm}If $A$ and $B$ are independent events, $P(A \cap B) = P(A)P(B)$.
\end{thm}

\rmk In the equation above, we could isolate $P(B)$ to obtain $P(B) = P(B \given A)$. Independence is a symmetric relation. You should interpret Definition \ref{independentevents} as `the probability of one event occurring is not influenced by the occurrence of the other' without worrying about memorizing which symbol goes where.
