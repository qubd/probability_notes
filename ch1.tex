...
\par
\begin{itemize}
\item $P(A \cup B) = P(A) + P(B) \text{ if $A$ and $B$ are mutually exclusive.}$
\item $P(A^c) = 1- P(A) \text{ for any event $A$.}$
\item $P(A \cup B) = P(A) + P(B) - P(A \cap B) \text{ for any events $A$ and $B$.}$
\end{itemize}
...
\eqns{P(E \given F) = \frac{P(E \cap F)}{P(F)}}
\end{defn}
...
\par
Intuitively, if $A$ and $B$ both occur, then $A$ must occur and $B$ must occur in this new setting where the event $A$ has already happened. The example below illustrates this idea well.
\begin{examp}
...
$$P(B) = P(B \given A_1)P(A_1) + P(B \given A_2)P(A_2)+ \, ... \, + P(B \given A_n)P(A_n).$$
\noindent Note that we require $P(A_i) > 0$ for each $A_i$, so that the conditional probabilities on the right are defined.
\end{thm}
\par
When a sequence $\{A_i\}_{i=1}^{n}$ is mutually exclusive sequence and has $\bigcup_{i=1}^{n}A_i = \Omega$, we say that the sequence \emx{partitions the sample space}. This means that every possible outcome is in exactly one of the $A_i$.
...
\eqnsgap{P(B) = P(B \given A)P(A) + P(B \given A^c)P(A^c)}
\end{cor}
...
(Bayes' Rule) If $\{A_i\}_{i=1}^n$ is a sequence of events which partitions the sample space $\Omega$, and $B$ is an event,
\eqnsgap{P(A_i \given B) = \frac{P(B \given A_i)P(A_i)}{P(B \given A_1)P(A_1) + \, ... \, + P(B \given A_n)P(A_n)}}
\end{thm}
\par
\begin{pf} Starting with the definition of conditional probability, using the intersection rule in the numerator, and the law of total probability in the denominator,
\eqns{P(A_i \given B) &= \frac{P(A_i \cap B)}{P(B)} \\
&= \frac{P(A_i)P(B \given A_i)}{P(B)} = \frac{P(B \given A_i)P(A_i)}{P(B)}   \\
&= \frac{P(B \given A_i)P(A_i)}{P(B \given A_1)P(A_1) +  \, ... \, + P(B \given A_n)P(A_n)}}
\end{pf}
\par
\rmk There are many ways to rewrite Bayes' rule. The reason it's shown in the longer form above is that in many applications, this form is most immediately applicable, as we'll see in examples below. Intuitively, the numerator includes only branches of the tree diagram where both $A_i$ and $B$ occur, while the denominator includes all branches where $B$ occurs.
\begin{cor}
(Bayes' Rule, Complement Case) If $A$ and $B$ are any events,
\eqnsgap{P(A \given B) = \frac{P(B \given A)P(A)}{P(B \given A)P(A) + P(B \given A^c)P(A^c)}}
\end{cor}
\begin{pf}
The sequence $\{A, A^c\}$ partitions $\Omega$ for any event $A$.
\end{pf}
\begin{examp}
...
\par
\noindent Let $D =$ `the randomly selected individual tested carries the disease', and $P =$ `the result of the test is positive'. Using Bayes' rule, we compute $P(D \given P)$.
...
\par
\noindent Thus, if a randomly selected individual tests positive, there's only a $33\%$ chance they carry the disease. Despite the positive test result, it's still more likely that they don't carry the disease. 
\end{examp}
\par
This example illustrates the problem of over-testing. If data from medical tests is to be useful, they should be applied carefully, often only in cases where there's some reason to believe the patient has an elevated risk of carrying the disease. As the test is more widely applied, the disease becomes more unusual in the group being tested and false positives become more likely. To illustrate this point, consider the most extreme case, where we're testing for a disease which has no cases in the group being tested. Every positive test will be a false positive, regardless of how accurate the test might be.
\par
It's tempting to assess the usefulness of the test by considering only the cited 99\% accuracy and ignoring the context in which the test is applied (the frequency with which the disease occurs in the population being tested). This is known as the \emx{base rate fallacy}\index{Base Rate Fallacy} and is a very common reasoning error. As we saw above, even an extremely accurate test will be nearly useless when applied to a population where the disease almost never occurs.
\begin{examp}
...
\end{examp}
\par
If we write Bayes' rule without using the law of total probability to expand the denominator, we obtain the variation below, which illustrates well the terminology you'll often hear associated with Bayes' rule. 
\eqns{P(A \given B) = \frac{P(B \given A)P(A)}{P(B)}}
\par
When employing Bayes' rule, it's often pertinent to think of $B$ as some evidence which we're using to reassess the probability of $A$. The quantity $P(A)$ is called a \emx{prior probability}\index{Prior Probability} since it represents the probability of $A$ before observing the new evidence, while $P(A \given B)$ is known as a \emx{posterior probability}\index{Posterior Probability}.
\par
We're now better equipped to understand the Bayesian interpretation of probability given in Section \ref{ProbInterpretationSection}. Two individuals with the same prior beliefs who observe the same evidence should agree on what constitutes a rational decision. However, it allows for two individuals who have different priors to disagree. This dependence on the knowledge / assumptions / guesses we have before the experiment occurs allows the Bayesian interpretation to apply in cases where the event we're interested in is not part of an indefinitely repeatable procedure.
% Do lost keys in three classes example as an exercise
