...
\par
\begin{defn}\label{FormalSRSDef}\index{Simple Random Sample} (SRS Formally) Given a distribution $X$, a simple random sample taken with replacement from $X$ is a sequence $X_1, X_2, \dots \,, X_n$ of independent, identically distributed random variables, each of which has the same distribution as $X$.
\end{defn}
...
If $\overline{X} = \frac{1}{n}(X_1+X_2+\,\cdots\,+X_n)$ is the mean of a SRS taken from a distribution with mean $\mu$ and variance $\sigma^2$, then $E(\overline{X}) = \mu$, and $\Var(\overline{X}) = \frac{\sigma^2}{n}$.
\end{thm}
...
Intuitively, we would expect that the mean of a random sample would give a good estimate of the mean of the population the sample was drawn from. The result above shows that indeed, this estimator is unbiased.
\par
Notice that as $n$ grows, $\Var(\overline{X}) = \frac{\sigma^2}{n}$ decreases. This is what we should expect. The larger the sample size, the narrower the sampling distribution of $\overline{X}$, that is, the mean of a larger sample gives a better estimate of $\mu$. We now have a precise statement that formalizes this intuition.
