...
\par
\begin{thm} (Central Limit Theorem)\index{Central Limit Theorem}\label{CLT} Suppose $X$ is a random variable with mean $\mu$ and standard deviation $\sigma$, and let $\xbar$ denote the mean of a sample of $n$ values drawn from the distribution of $X$. Then as the sample size $n$ grows larger, the sampling distribution of $\xbar$ becomes better approximated by $Normal(\muxbar,\sigmaxbar)$.
\end{thm}
\par
