...
%Formally, if $X_1, X_2, ..., X_n$ are the $n$ sample values, let $S_n$ be a random variable whose value is given by uniformly selecting an integer $i$ between $1$ and $n$ inclusive, and letting $S_n = X_i$, so that $S_n$ represents a randomly selected value in our sample.
...
\par
\begin{thm} (Central Limit Theorem)\index{Central Limit Theorem}\label{CLT} Let $X$ be a random variable with mean $\mu$ and standard deviation $\sigma$, and let $\xbar$ denote the mean of a simple random sample of $n$ values drawn from the distribution of $X$. Then as the sample size $n$ grows larger, the sampling distribution of $\xbar$ becomes better approximated by $Normal(\muxbar,\sigmaxbar)$.
\end{thm}
\par
The conclusion of this theorem is written here in very informal language, which is sufficient for our purposes. To make it more precise, we would need to decide on a way to measure the difference between two random variables, so the theorem above could state that this difference vanishes as $n \to \infty$. For details see \cite{Bertsekas} or \cite{DevoreBerk}.
